...
export const WebhookSalesLogsView = () => {
  ...
  useLayoutEffect(() => {
    if (currentViewName !== "Webhook Sales Logs") {
      handleChangeViewName("Webhook Sales Logs");
    }
  }, [currentViewName, handleChangeViewName]);
  const { data: gridData, refetch: refetchGridData } = useQuery({
    queryKey: ["webhook-sales-logs-data"],
    queryFn: async () => await webhookService.GetSalesLogs(),
    initialData: [],
  ...
  const handleReintent = useCallback(
    async (data: webhookLogs) => {
      const res = await webhookService.ReintentSalesLog(data.id);
      if (res) {
